 ...
 * @author DrunkenMan
 */
public class LaserMonitor extends Thread
{
	public LaserMonitor(LaserGUI gui, JasonECL laser)
	{
		this.gui = gui;
		this.laser = laser;
		this.home = false;
	...
	public void run()
	{
		try
		{
			...
			do
			{
				...
				// If monitoring, find out where the laser is at
				gui.setRealWave(laser.getLambda());
				...
				if (nextSetWave != curSetWave)
				{
					...
					laser.setLambda(nextSetWave, false);				
				}
				Thread.sleep(300);
			} while (!finished);
