...
class DSOX4104A:
    ...
    def setChannelOption(self, channel = 1, coupling = "DC", bwlimit = 0, display = 1, \
        ...
        self.scope.write(":chan%d:scal %fmV" % (channel, scal))
    ...
    def takeWaveform(self, numCycles = 1, channel = 1, timerange = 1e-7, wformat = "ascii", \
        ...
        time_span = time_step * int(self.scope.query(":wav:poin?"))
        ...
        waveform = self.scope.query(":WAVEFORM:DATA?")
        # remove preamble
        ...
        for i in range(numCycles-1):
            self.scope.clear()
            self.scope.write(":sing")
            newwave = self.scope.query(":WAVEFORM:DATA?")
