...
from KC705Packets import GTXTxPacket
from KC705Packets import GTXRxPacket
from KC705GTXDRP import *

# Class with helper functions to control an KC705 GTX transceiver
class KC705GTX:
    ...
    # Read a DRP register
    def read_drp(self, gtx_num, address):
        # Some checks
        if (gtx_num < 0) or (gtx_num > 4):
            raise Exception("GTX number must be between 0 and 4")
        if (address < 0) or (address > 512):
            raise Exception("DRP register addresses must be between 0 and 255")

        rx_packet = GTXRxPacket()
        self.eth.send(GTXTxPacket(command = binary_repr(gtx_num, 3) + '0', address = binary_repr(address, 9)))
        self.eth.recv(rx_packet)
        return rx_packet.data
        
    # Write a DRP register
    def write_drp(self, gtx_num, address, data):
        # Some checks
        if (gtx_num < 0) or (gtx_num > 4):
            raise Exception("GTX number must be between 0 and 4")
        if (address < 0) or (address > 512):
        ...
        rx_packet = GTXRxPacket()
        self.eth.send(GTXTxPacket(command = binary_repr(gtx_num, 3) + '1', address = binary_repr(address, 9), data = data))
        self.eth.recv(rx_packet)
        return rx_packet.data
