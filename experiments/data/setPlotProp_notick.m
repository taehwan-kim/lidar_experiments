...
% turn off right/top ticks
set( haxis,'box','off','color','none');
tempaxes = axes('Position',get(haxis,'Position'),'color','none','box','on','xtick',[],'ytick',[]);
axes(haxis);
linkaxes([haxis tempaxes]);
% haxistemp = haxis;
% 
% haxis = [haxis tempaxes];
% 
% hxl = [hxl get(tempaxes, 'XLabel')];
% hyl = [hyl get(tempaxes, 'YLabel')];
% hzl = [hyl get(tempaxes, 'ZLabel')];
